...
(function ($) {
    ...
    $.fn.simpleCropper = function (onComplete) {
        ...
        var jcrop_api;
        var bottom_html = "<input type='file' id='fileInput' class='fileinputgroup' name='files[]'/ accept='image/*'><canvas id='myCanvas' style='display:none;'></canvas><div id='modal'></div><div id='preview'><div class='buttons'><div class='cancel'></div><div class='ok'></div></div></div>";
        $('#image-container').html(bottom_html);
        var $imageItem = this;
        ...
        function preview() {
            ...
            $($imageItem).append('<input type="text" id="image-name-' + $imageId + '" class="form-control" value="' + image_filename + '"/>');
            var idImage = "#image-name-";
            var valImageId = $(idImage + idImage).data("imageid");
            var deleteInput = '<button class="btn btn-danger"  style="position: relative;top:-63px;" onClick=\"return fnDeleteNewImage(' + $imageId + ',\'#image-name-' + $imageId + '\');\"><i class="la la-trash-o"></i></button>';
            var saveInput = '<button class="btn btn-primary"  style="position: relative;top:-63px;left:82px;" onClick="return fnUpdateImageName(\'#image-name-' + $imageId + '\');"><i class="la la-save"></i></button>';

            var changeStatus = '<button class="btn btn-primary"  id=image-name-' + $imageId+' style="position: relative; top: -63px; left: -26px;" onClick="return fnChangeImageStatusForClass(\'#image-name-' + $imageId + '\');"><i class="fa fa-check-circle"></i></button>';
           
           
            $($imageItem).append(deleteInput);
                $($imageItem).append(saveInput);
                $($imageItem).append(changeStatus);
